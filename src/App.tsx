...
export default function App() {
  ...
  useEffect(() => {
    ...
    const scroll = () => {
      ...
      if (next <= 8) setCompact(false);
      else if (next - previous > 6 && next > 100) setCompact(true);
      else if (previous - next > 6) setCompact(false);
      previous = next;
    };
    ...
    <header className={`site-header ${compact ? 'is-compact' : ''}`}>
      <div className="page-width">
        <div className="brand-row">
          ...
          <div className="brand-copy"><h1 className="brand-title">World of Ships Balance Changes</h1>{!compact && <p>舰船平衡改动总览</p>}</div>
          <div className="header-actions"><AppearanceControl /><button className="icon-button" aria-label={compact ? '展开页头' : '折叠页头'} aria-expanded={!compact} onClick={() => setCompact(!compact)}><ChevronDown size={20} className={compact ? '' : 'rotate-180'} /></button></div>
        </div>
        {!compact && <div className="meta-grid">
